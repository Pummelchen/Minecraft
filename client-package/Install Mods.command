...
MOD_COUNT="$(find "$MC_DIR/mods" -maxdepth 1 -type f -name '*.jar' | wc -l | tr -d '[:space:]')"
RP_COUNT="$(find "$MC_DIR/resourcepacks" -maxdepth 1 -type f \( -name '*.zip' -o -name '*.jar' \) | wc -l | tr -d '[:space:]')"
SHADER_COUNT="$(find "$MC_DIR/shaderpacks" -maxdepth 1 -type f \( -name '*.zip' -o -name '*.jar' \) | wc -l | tr -d '[:space:]')"
